...
class ReadDataframes(spark: SparkSession) extends ReadDFs {
  ...
  override def read_hdfs_df(file_path: String, file_format: String="csv", schema: StructType= new StructType(),options: Map[String,String]=Map(), clause: String="", columns_to_read: List[String]=Nil): DataFrame = {
    if (options.isEmpty) {
